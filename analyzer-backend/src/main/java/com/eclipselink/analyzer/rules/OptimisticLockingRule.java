    ...
    public List<Violation> check(EntityNode entity) {
        ...
        for (String attrName : entity.getAttributes().keySet()) {
            if (attrName.equalsIgnoreCase("version") || 
                ...
                attrName.toLowerCase().contains("lock")) {
                hasVersion = true;
                break;
            }
        }
        ...
        if (!hasVersion) {
            violations.add(new Violation(getId(), getSeverity(),
                    "Entity '" + entity.getName() + "' does not have a version field for optimistic locking. Consider adding @Version to prevent concurrent modification issues."));
        }
