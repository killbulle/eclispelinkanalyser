...
public class GraphAnalysisRule implements GlobalMappingRule {
    ...
    public List<MappingRule.Violation> checkAll(List<EntityNode> entities) {
        ...
        // Detect cycles
        List<String> cycles = graphAnalyzer.detectCycles();
        if (!cycles.isEmpty()) {
            for (String entityName : cycles) {
                violations.add(new MappingRule.Violation(getId(), "ERROR",
                        "Entity '" + entityName + "' is part of a cyclic dependency. " +
                        "Cycles can cause issues with serialization, transaction boundaries, and object loading. " +
                        "Consider breaking the cycle or using @XmlIDREF/@JsonManagedReference/@JsonBackReference annotations."));
            }
        }
